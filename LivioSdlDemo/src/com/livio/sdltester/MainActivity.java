...
import com.livio.sdl.ext.SdlRequestFactoryExt;
import com.livio.sdl.ext.SdlServiceExt;
...
public class MainActivity extends Activity {
	boolean addMyCommand = false;
	Logger logger = Logger.getLogger(MainActivity.class.getName());

	/*
	 * Target we publish for clients to send messages to IncomingHandler.
	 * 附带在发送消息的ReplyTo 里面，在返回响应时候可以处理具体的响应，IncomingHandler正是处理返回响应的函数
	 */
	final Messenger mMessenger = new Messenger(new IncomingHandler());
	/* Messenger for communicating with service. Activity　发送消息给service的工具 */
	private Messenger serviceMsgr = null;
	
	/*
	 * Class for interacting with the main interface of the service.
	 * 
	 * 当前Activity和它绑定的service的连接器定义
	 */
	private final ServiceConnection mConnection = new ServiceConnection() {
		public void onServiceConnected(ComponentName className, IBinder service) {
			// 要注意mConnection 是Activity和它绑定的service的连接器，而包含service的Messenger(serviceMsgr)
			// 则是Activity　发送消息给service的工具
			logger.info(" after connect service .......... ");
			serviceMsgr = new Messenger(service);// 注意它会指向 SdlService 的onBind方法返回的对象，因此MainActivity中发送的消息会被 SdlService的 IncomingHandler 接收 

			logger.info(" register client to service .......... ");
			Message msg = Message.obtain(null,
					SdlServiceExt.ServiceMessages.REGISTER_CLIENT);
			msg.replyTo = mMessenger;
			sendMessageToService(msg);

			logger.info(" show sdl connect dialog .......... ");
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SdlServiceExt.setDebug(true);
		// 将需要发送到车机的图片缓存到内存中.便以后面快速访问
		createImageCache();
		// 界面信息初始化，如连接状态，按钮内容等.
		init();
		// 绑定service
	...
	private static final class ResultCodes {
		private static final class SubmenuResult {
			...
			private static final int ADD_COMMAND_INIT = 2;
		}
	...
	private class IncomingHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			// see SdlServiceExt.java in the com.livio.sdl package for details
			// on
			// these incoming messages

			switch (msg.what) {
			case SdlServiceExt.ClientMessages.SDL_CONNECTED:
				...
				setAppIcon();
				break;
			case SdlServiceExt.ClientMessages.SDL_DISCONNECTED:
				resetService();
				updateConnectionStatus(ConnectionStatus.OFFLINE_MODE);
				clearSdlLog();
				break;
			case SdlServiceExt.ClientMessages.SDL_HMI_FIRST_DISPLAYED:
				setInitialHmi();
				break;
			case SdlServiceExt.ClientMessages.ON_MESSAGE_RESULT:
				onMessageResponseReceived((RPCMessage) msg.obj);
				break;
			case SdlServiceExt.ClientMessages.SUBMENU_LIST_RECEIVED:
				onSubmenuListReceived((List<MenuItem>) msg.obj, msg.arg1);
				break;
			case SdlServiceExt.ClientMessages.COMMAND_LIST_RECEIVED:
				onCommandListReceived((List<MenuItem>) msg.obj, msg.arg1);
				break;
			case SdlServiceExt.ClientMessages.BUTTON_SUBSCRIPTIONS_RECEIVED:
				onButtonSubscriptionsReceived((List<SdlButton>) msg.obj,
						msg.arg1);
				break;
			case SdlServiceExt.ClientMessages.INTERACTION_SETS_RECEIVED:
				onInteractionListReceived((List<MenuItem>) msg.obj, msg.arg1);
				break;
			case SdlServiceExt.ClientMessages.PUT_FILES_RECEIVED:
	...
	private void sendSdlMessageToService(RPCRequest request) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.SEND_MESSAGE);
		msg.obj = request;
		sendMessageToService(msg);
	}
	...
	private void sendSubmenuListRequest(int reqCode) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.REQUEST_SUBMENU_LIST);
		msg.replyTo = mMessenger;
		msg.arg1 = reqCode;
		sendMessageToService(msg);
	}
	...
	private void sendCommandListRequest(int reqCode) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.REQUEST_COMMAND_LIST);
		msg.replyTo = mMessenger;
		msg.arg1 = reqCode;
		sendMessageToService(msg);
	}
	...
	private void sendButtonSubscriptionRequest(int reqCode) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.REQUEST_BUTTON_SUBSCRIPTIONS);
		msg.replyTo = mMessenger;
		msg.arg1 = reqCode;
		sendMessageToService(msg);
	}
	...
	private void sendInteractionSetRequest(int reqCode) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.REQUEST_INTERACTION_SETS);
		msg.replyTo = mMessenger;
		msg.arg1 = reqCode;
		sendMessageToService(msg);
	}
	...
	private void sendPutFileRequest(int reqCode) {
		Message msg = Message.obtain(null,
				SdlServiceExt.ServiceMessages.REQUEST_PUT_FILES);
		msg.replyTo = mMessenger;
		msg.arg1 = reqCode;
		sendMessageToService(msg);
	}
	...
	private void resetService() {
		artworkSet = false;
		Intent sdlService = new Intent(MainActivity.this, SdlServiceExt.class);
	...
	/*
	 * Sets up the App's icon using PutFile & SetAppIcon commands 注意：是在车机端建立图片
	 */
	...
	private void setInitialHmi() {
		...
		// TODO my button
		SoftButton startBtn = new SoftButton();
		startBtn.setText("start");
		startBtn.setSystemAction(SystemAction.DEFAULT_ACTION);
		startBtn.setType(SoftButtonType.SBT_TEXT);
		startBtn.setSoftButtonID(1000);
		SoftButton endBtn = new SoftButton();
		endBtn.setText("end");
		endBtn.setSystemAction(SystemAction.DEFAULT_ACTION);
		endBtn.setType(SoftButtonType.SBT_TEXT);
		endBtn.setSoftButtonID(1001);
		List<SoftButton> sbs = new ArrayList<SoftButton>();
		sbs.add(startBtn);
		sbs.add(endBtn);

		// create & send the Show command
		RPCRequest showMsg = SdlRequestFactoryExt.show(showText1, showText2,
		...
		RPCRequest firstAddSubMenuMsg = SdlRequestFactoryExt.addSubmenu("地区",
				1, 1100);
		RPCRequest secondAddSubMenuMsg = SdlRequestFactoryExt.addSubmenu("频道",
				2, 1101);
		sendSdlMessageToService(firstAddSubMenuMsg);
		sendSdlMessageToService(secondAddSubMenuMsg);

		// AddSubMenu menu1 = (AddSubMenu) firstAddSubMenuMsg;
		// AddSubMenu menu2 = (AddSubMenu) secondAddSubMenuMsg;
		// logger.info(menu1.getMenuName() + ":　" + menu1.getMenuID());
		// logger.info(menu2.getMenuName() + ":　" + menu2.getMenuID());
		// RPCRequest addCommand1Menu1 = SdlRequestFactory.addCommand("上海", 1,
		// menu1.getMenuID(), "", null);
		// RPCRequest addCommand2Menu1 = SdlRequestFactory.addCommand("江西", 2,
		// menu1.getMenuID(), "", null);
		// RPCRequest addCommand3Menu1 = SdlRequestFactory.addCommand("湖南", 3,
		// menu1.getMenuID(), "", null);
		// RPCRequest addCommand4Menu1 = SdlRequestFactory.addCommand("湖北", 4,
		// menu1.getMenuID(), "", null);
		//
		// RPCRequest addCommand1Menu2 = SdlRequestFactory.addCommand("直播", 1,
		// menu2.getMenuID(), "", null);
		// RPCRequest addCommand2Menu2 = SdlRequestFactory.addCommand("点播", 2,
		// menu2.getMenuID(), "", null);
		// sendSdlMessageToService(addCommand1Menu1);
		// sendSdlMessageToService(addCommand2Menu1);
		// sendSdlMessageToService(addCommand3Menu1);
		// sendSdlMessageToService(addCommand4Menu1);
		// sendSdlMessageToService(addCommand1Menu2);
		// sendSdlMessageToService(addCommand2Menu2);

	}

	private void addMyCommand() {
		RPCRequest addCommand1Menu1 = SdlRequestFactory.addCommand("文件一", 1, 0,
				"", null);
		RPCRequest addCommand2Menu1 = SdlRequestFactory.addCommand("文件二", 2, 0,
				"", null);
		RPCRequest addCommand3Menu1 = SdlRequestFactory.addCommand("文件三", 3, 0,
				"", null);
		RPCRequest addCommand4Menu1 = SdlRequestFactory.addCommand("文件四", 4, 0,
				"", null);
	}

	private void addMyCommandToMenu(List<MenuItem> submenuList) {
		for (MenuItem menuItem : submenuList) {
			logger.info(menuItem.getName() + " id is " + menuItem.getId());
		}
		if (!addMyCommand) {
			int menu1Id = 1100;
			int menu2Id = 1101;
			if (submenuList.size() == 2) {
				menu1Id = submenuList.get(0).getId();
				menu2Id = submenuList.get(1).getId();
			}
	...
	private void doBindService() {
		logger.info("start bind service .......... ");
		if (!isBound) {
			bindService(new Intent(MainActivity.this, SdlServiceExt.class),
	...
	private void doUnbindService() {
		if (isBound) {
			if (serviceMsgr != null) {
				Message msg = Message.obtain(null,
						SdlServiceExt.ServiceMessages.UNREGISTER_CLIENT);
				msg.replyTo = mMessenger;
				sendMessageToService(msg);
			}
			...
			stopService(new Intent(MainActivity.this, SdlServiceExt.class));
			isBound = false;
		}
	}

	// create an image cache for the images that are available to send to the
	// head-unit. this allows easy image look-up
	// based on the filename of the image. We'll do this on a thread since
	// processing images can be CPU intensive.
		...
		findViewById(R.id.btn_main_sendMessage).setOnClickListener(
				new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						// lazily instantiate the list of commands in
						// alphabetical order
						if (commandList == null) {
							// grab the command list and sort the commands by
							// name
							commandList = Arrays.asList(SdlCommand.values());
							Collections.sort(commandList,
									new EnumComparator<SdlCommand>());
						}

						// show the dialog with the commandList we created above
						BaseAlertDialog commandDialog = new ListViewDialog<SdlCommand>(
								context, dialogTitle, commandList);
						commandDialog
								.setListener(new BaseAlertDialog.Listener() {
									@Override
									public void onResult(Object resultData) {
										// when a list item is clicked, dispatch
										// the click to the showCommandDialog
										// method
	...
	private void updateConnectionStatus(ConnectionStatus status) {
		...
		if (status == ConnectionStatus.OFFLINE_MODE) {
			sendMessageToService(Message.obtain(null,
					SdlServiceExt.ServiceMessages.OFFLINE_MODE));
		}
	...
	protected void onDestroy() {
		sendMessageToService(Message.obtain(null,
				SdlServiceExt.ServiceMessages.DISCONNECT));
		doUnbindService();
		super.onDestroy();
	}
	...
	private void onMessageResponseReceived(RPCMessage response) {
		...
		// this.addMyCommand();
		logger.info("response is " + response.getClass().getName());
		// if (response instanceof OnButtonEvent) {
		// logger.info("response is OnButtonEvent");
		// OnButtonEvent btnEvt = (OnButtonEvent) response;
		// logger.info("btnEvt is " + btnEvt.getButtonName());
		// SdlButton sdlButton =
		// SdlButton.translateFromLegacy(btnEvt.getButtonName());
		// String text = new
		// StringBuilder().append(sdlButton.toString()).append(" clicked!").toString();
		// logger.info(text);
		// }
		// AddSubMenuResponse

		// TODO 获取车机端Menu 列表
		if (response instanceof OnHMIStatus) {
			OnHMIStatus newStatus = (OnHMIStatus) response;
			if (newStatus.getHmiLevel() == HMILevel.HMI_FULL
					&& newStatus.getSystemContext() == SystemContext.SYSCTXT_MENU) {
				sendSubmenuListRequest(ResultCodes.SubmenuResult.ADD_COMMAND_INIT);
			}
		}

		if (response instanceof OnButtonPress) {
			logger.info("response is OnButtonPress");
			OnButtonPress btnEvt = (OnButtonPress) response;
			logger.info("btnEvt is " + btnEvt.getButtonName());
			SdlButton sdlButton = SdlButton.translateFromLegacy(btnEvt
					.getButtonName());
			String text = new StringBuilder().append(sdlButton.toString())
		...
		// logger.info("btnEvt is " + btnEvt.getButtonName());
		//
		// if (btnEvt.getButtonName().equals(
		// ButtonName.valueForString("MySoftBtn"))) {
		// String text = new StringBuilder().append("MySoftBtn")
		// .append(" Event Received!").toString();
		// showToast(text);
		// }
		//
		// if (btnEvt.getButtonName().equals(
		// ButtonName.valueForString("start"))
		// || btnEvt.getButtonName().equals(
		// ButtonName.valueForString("end"))) {
		// String text = new StringBuilder().append("start/end ")
		// .append(" Event Received!").toString();
		// showToast(text);
		// }
		// }
	...
	private void onSubmenuListReceived(List<MenuItem> submenuList, int reqCode) {
		Collections.sort(submenuList, new MenuItem.NameComparator()); // sort
																		// submenu
																		// list
																		// by
																		// name.
		...
		case ResultCodes.SubmenuResult.ADD_COMMAND_DIALOG:
			...
			break;
		case ResultCodes.SubmenuResult.ADD_COMMAND_INIT:
			// TODO 获取到列表后的行为
			submenuCache = submenuList;
			this.addMyCommandToMenu(submenuList);
			// sendPutFileRequest(ResultCodes.PutFileResult.ADD_COMMAND);
			break;
		case ResultCodes.SubmenuResult.DELETE_SUBMENU_DIALOG:
			if (submenuList.size() > 0) {
				...
				Toast.makeText(
						MainActivity.this,
						getResources()
								.getString(R.string.no_submenus_to_delete),
	...
	private void onCommandListReceived(List<MenuItem> commandList, int reqCode) {
		Collections.sort(commandList, new MenuItem.NameComparator()); // sort
																		// command
																		// list
																		// by
																		// name
		...
		case ResultCodes.CommandResult.DELETE_COMMAND_DIALOG:
			if (commandList.size() > 0) {
				...
				Toast.makeText(
						MainActivity.this,
						getResources()
								.getString(R.string.no_commands_to_delete),
						Toast.LENGTH_SHORT).show();
			}
			break;
		default:
		...
		case ResultCodes.ButtonSubscriptionResult.BUTTON_SUBSCRIBE:
			if (buttonSubscriptionList.size() == SdlButton.values().length) {
				Toast.makeText(
						...
						getResources()
								.getString(
										R.string.button_subscriptions_already_subscribed),
						Toast.LENGTH_LONG).show();
			} else {
				List<SdlButton> buttonsNotSubscribedTo = filterSubscribedButtons(buttonSubscriptionList);
				Collections.sort(buttonsNotSubscribedTo,
						new EnumComparator<SdlButton>());
			...
			if (buttonSubscriptionList.size() == 0) {
				Toast.makeText(
						...
						getResources().getString(
								R.string.button_subscriptions_none_subscribed),
						Toast.LENGTH_LONG).show();
			} else {
				Collections.sort(buttonSubscriptionList,
						new EnumComparator<SdlButton>());
		...
		case ResultCodes.InteractionSetResult.PERFORM_INTERACTION:
			if (interactionSetList.size() == 0) {
				Toast.makeText(
						MainActivity.this,
						getResources().getString(
								R.string.interaction_list_none_added),
						Toast.LENGTH_LONG).show();
			} else {
				Collections.sort(interactionSetList,
						new MenuItem.IdComparator());
	...
	private void onPutFileListReceived(List<String> putFileList, int reqCode) {
		...
		case ResultCodes.PutFileResult.PUT_FILE:
			...
			if (availableItems.size() > 0) {
				...
				Toast.makeText(this, "All images have been added!",
						Toast.LENGTH_LONG).show();
			}
			break;
		case ResultCodes.PutFileResult.DELETE_FILE:
			...
			if (availableItems.size() > 0) {
				...
				Toast.makeText(this, "No images have been added!",
						Toast.LENGTH_LONG).show();
			}
			break;
	 ...
	 * address for the core component they would like to connect to. 显示 sdl
	 * connection 界面窗口 <b>IMPORTANT NOTE</b>
	 * <p>
		...
		connectionDialog.setListener(new BaseAlertDialog.Listener() {
			...
			public void onResult(Object resultData) {
				...
				if (result == null || (ipAddressValid && ipPortValid)) {
					...
					Message msg = Message.obtain(null,
							SdlServiceExt.ServiceMessages.CONNECT);
					msg.obj = result;
					// TODO 通知service连接车机
					sendMessageToService(msg);
				}
	 ...
	 * 点击本地command触发对应的行为， 显示对应的 sdl command 界面窗口
	 * 
	 * @param command
	...
	private final BaseAlertDialog.Listener singleMessageListener = new BaseAlertDialog.Listener() {
		...
		public void onResult(final Object resultData) {
			// resultData在每个具体的dialog中构建，本地点击dialog的按钮后触发此处事件
			if (resultData != null) {
				new Thread(new Runnable() {
					@Override
					public void run() {
	...
	/************
	 * 具体的 SDL Command界面窗口 /** Creates an alert dialog, allowing the user to
	 * manually send an alert command.
	 */
	private void createAlertDialog(List<SdlImageItem> images) {
		BaseAlertDialog alertDialog = new SdlAlertDialog(this, images);
		alertDialog.setListener(singleMessageListener); // 点OK触发
														// singleMessageListener
		alertDialog.show();
	}

	/**
	 * Creates a speak dialog, allowing the user to manually send a speak
	 * command.
	 */
	...
	/**
	 * Creates a button subscribe dialog, allowing the user to manually send a
	 * button subscribe command.
	 * 
	 * @param buttonSubscriptions
	 *            The list used to populate the dialog
	 */
	private void createButtonSubscribeDialog(List<SdlButton> buttonSubscriptions) {
		BaseAlertDialog buttonSubscribeDialog = new ButtonSubscriptionDialog(
	...
	/**
	 * Creates a button unsubscribe dialog, allowing the user to manually send a
	 * button unsubscribe command.
	 * 
	 * @param buttonSubscriptions
	 *            The list used to populate the dialog
	 */
	private void createButtonUnsubscribeDialog(
			List<SdlButton> buttonSubscriptions) {
		BaseAlertDialog buttonUnsubscribeDialog = new ButtonUnsubscriptionDialog(
				this, buttonSubscriptions);
	...
	/**
	 * Creates an add command dialog, allowing the user to manually send an add
	 * command command.
	 * 
	 * @param allBanks
	 *            The list used to populate the dialog
	...
	/**
	 * Creates an add submenu dialog, allowing the user to manually send an add
	 * submenu command.
	 */
	...
	/**
	 * Creates a create interaction choice set dialog, allowing the user to
	 * manually send a create interaction choice set command.
	 */
	private void createInteractionChoiceSetDialog(List<SdlImageItem> addedImages) {
		BaseAlertDialog createInteractionChoiceSetDialog = new CreateInteractionChoiceSetDialog(
				this, addedImages);
	...
	/**
	 * Creates a change registration dialog, allowing the user to manually send
	 * a change registration command.
	 */
	private void createChangeRegistrationDialog() {
		BaseAlertDialog changeRegistrationDialog = new ChangeRegistrationDialog(
				this);
	...
	/**
	 * Creates a delete command dialog, allowing the user to manually send a
	...
	private void createDeleteCommandDialog(List<MenuItem> commandList) {
		BaseAlertDialog deleteCommandDialog = new DeleteCommandDialog(this,
				commandList);
	...
	/**
	 * Creates a delete submenu dialog, allowing the user to manually send a
	 * delete submenu command.
	 * 
	 * @param submenuList
	 *            The list used to populate the dialog
	 */
	private void createDeleteSubmenuDialog(List<MenuItem> submenuList) {
		BaseAlertDialog deleteCommandDialog = new DeleteSubmenuDialog(this,
				submenuList);
		deleteCommandDialog.setListener(singleMessageListener);
		deleteCommandDialog.show();
	}
	...
	/**
	 * Creates a perform interaction dialog, allowing the user to manually send
	 * a PerformInteraction command.
	 * 
	 * @param interactionList
	 *            The list used to populate the dialog
	 */
	private void createPerformInteractionDialog(List<MenuItem> interactionList) {
		BaseAlertDialog performInteractionDialog = new PerformInteractionDialog(
	...
	/**
	 * Creates a delete interaction dialog, allowing the user to manually send a
	 * DeleteInteractionChoiceSet command.
	 * 
	 * @param interactionList
	 *            The list used to populate the dialog
	 */
	private void createDeleteInteractionDialog(List<MenuItem> interactionList) {
		BaseAlertDialog deleteInteractionDialog = new DeleteInteractionDialog(
				this, interactionList);
	...
	/**
	 * Creates a get DTCs dialog, allowing the user to manually send a GetDTCs
	 * command.
	 */
	...
	/**
	 * Creates a read DIDs dialog, allowing the user to manually send a ReadDID
	 * command.
	 */
	...
	/**
	 * Creates a slider dialog, allowing the user to manually send a Slider
	 * command.
	 */
	...
	/**
	 * Creates a scrollable message dialog, allowing the user to manually send a
	 * ScrollableMessage command.
	 */
	private void createScrollableMessageDialog(List<SdlImageItem> images) {
		BaseAlertDialog scrollableMessageDialog = new ScrollableMessageDialog(
				this, images);
	...
	/**
	 * Creates a set media clock timer dialog, allowing the user to manually
	 * send a SetMediaClockTimer command.
	 */
	private void createSetMediaClockTimerDialog() {
		BaseAlertDialog setMediaClockTimerDialog = new SetMediaClockTimerDialog(
				this);
	...
	/**
	 * Creates a put file dialog, allowing the user to manually send images
	 * through the PutFile command.
	 * 
	 * @param imagesAddedSoFar
	 *            Images that have <b>not</b> already been added
	 */
	private void createPutFileDialog(List<SdlImageItem> imagesAddedSoFar) {
		BaseAlertDialog putFileDialog = new PutFileDialog(this,
				imagesAddedSoFar);
	...
	/**
	 * Creates a delete file dialog, allowing the user to manually delete files
	 * that have been added through the PutFile command.
	 * 
	 * @param imagesAddedSoFar
	 *            The list of images that have been added so far
	 */
	private void createDeleteFileDialog(List<SdlImageItem> imagesAddedSoFar) {
		BaseAlertDialog deleteFileDialog = new DeleteFileDialog(this,
				imagesAddedSoFar);
	...
	/**
	 * Creates a set app icon dialog, allowing the user to manually set their
	 * app icon based on images that have been added through the PutFile
	 * command.
	 * 
	 * @param imagesAddedSoFar
	 *            The list of images that have been added so far
	...
	private void createSetAppIconDialog(List<SdlImageItem> imagesAddedSoFar) {
		BaseAlertDialog setAppIconDialog = new SetAppIconDialog(this,
				imagesAddedSoFar);
	...
	public boolean onOptionsItemSelected(android.view.MenuItem item) {
		...
		case R.id.menu_reset:
			artworkSet = false;
			sendMessageToService(Message.obtain(null,
					SdlServiceExt.ServiceMessages.RESET));
			return true;
