...
/**
 * 一个管道服务器，端口从7474开始,尝试20次,每次+1,一直到找到可用端口。 然后建立ServerSocket,等待客户端连接.
 * 客户端连接后，会得到一个连接的socket(m_siphonSocket). 
 * 
 * 该Server主要是监听客户端连接，并发送消息给客户端。不接收客户端消息.
 * @author html5app
 *
 */
public class SiphonServer {
	...
	public static Boolean sendBytesFromSDL(byte[] msgBytes, int offset, int length) {		
		if (m_sendingFormattedTrace) {
			DebugTool.logInfo("m_sendingFormattedTrace is true , so return false");
			return false;
		}else{
			DebugTool.logInfo("m_sendingFormattedTrace is false, continue ");
		}
	...
	private synchronized static Boolean sendDataToSiphonSocket(byte directionIndicator, Integer timeStamp, 
		...
		if (!m_siphonEnabled) {
			DebugTool.logInfo("m_siphonEnabled in [sendDataToSiphonSocket] is false, so return ... ");
			return false;
		}
		
		if (msgBytes == null || length == 0) {
			DebugTool.logInfo("msg in [sendDataToSiphonSocket] is empty, so return ... ");
			return false;
		}
		...
		if (siphonOutputStream == null) {
			DebugTool.logInfo("siphonOutputStream in [sendDataToSiphonSocket] is null, so return ... ");
			return false;
		}		
		
		DebugTool.logInfo("start sendDataToSiphonSocket in [sendDataToSiphonSocket] ... ");
		try	{		
			...
			DebugTool.logInfo( ex.getMessage() + "　in [sendDataToSiphonSocket] ... ");
			return false;
		} // end-catch
